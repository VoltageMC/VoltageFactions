...
class EnchantUtils
{
    ...
    const RARITY_NAMES = [
        CustomEnchant::RARITY_SIMPLE => "Simple",
        ...
        CustomEnchant::RARITY_ENERGY => "Energy",
        CustomEnchant::RARITY_EXECUTIVE => 'Executive'
    ];

    const INCOMPATIBLE_ENCHANTS = [
        EnchantIds::BLAZE => [EnchantIds::PORKIFIED, EnchantIds::WITHERSKULL],
        EnchantIds::GRAPPLING => [EnchantIds::VOLLEY],
        EnchantIds::GROW => [EnchantIds::SHRINK],
        EnchantIds::HOMING => [EnchantIds::BLAZE, EnchantIds::PORKIFIED, EnchantIds::WITHERSKULL],
        EnchantIds::PORKIFIED => [EnchantIds::WITHERSKULL]
    ];
        ...
        if (count($item->getEnchantments()) > 0) {
            ...
            foreach ($item->getEnchantments() as $enchantmentInstance) {
                ...
                if ($enchantment instanceof CustomEnchant) {
                    $additionalInformation .= "\n" . TextFormat::RESET . EnchantUtils::getColorFromRarity($enchantment->getRarity()) . $enchantment->getDisplayName() . " " . ($plugin->getConfig()->getNested("enchants.roman-numerals") ? EnchantUtils::getRomanNumeral($enchantmentInstance->getLevel()) : $enchantmentInstance->getLevel());
                }
            }
            ...
            if (EnchantAPI::getPlugin()->getConfig()->getNested("enchants.position") === "lore") {
    ...
    {
        return self::getTFConstFromString(EnchantAPI::getPlugin()->getConfig()->get("rarity-colors")[strtolower(self::RARITY_NAMES[$rarity])]);
    }
